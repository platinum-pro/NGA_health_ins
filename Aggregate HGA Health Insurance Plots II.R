...
# Modified create_plot function with APA-style panel labels
create_demographic_plot <- function(data, var_prefix, label1, label0, panel_letter = "") {
  ...
  # Add APA-style panel label using plot title (most reliable method)
  if(panel_letter != "") {
    p <- p + 
      labs(
        title = panel_letter,  # Panel letter as title
        color = "", shape = "", linetype = ""
      ) +
      theme(
        plot.title = element_text(size = 14, face = "bold", hjust = 0, 
                                  margin = margin(0, 0, 5, 0))  # Left-aligned, minimal bottom margin
      )
  } else {
    p <- p + 
      labs(
...
# Create all plots with panel letters
plots <- list()
panel_letters <- LETTERS[1:length(demographic_pairs)]  # A, B, C, D, etc.

for(i in seq_along(demographic_pairs)) {
  demo <- demographic_pairs[[i]]
  plots[[demo$var]] <- create_demographic_plot(data, demo$var, demo$label1, demo$label0, panel_letters[i])
}
