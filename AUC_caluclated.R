...
# Calculate AUC scores and add them as a new column
data$normalized_auc <- apply(data, 1, calculate_normalized_auc)

# Format the AUC values as decimals
data$normalized_auc <- format(data$normalized_auc, scientific = FALSE, digits = 10)

# Save the updated dataset back to CSV
write.csv(data, "~/Downloads/Health_Ins_with_AUC.csv", row.names = FALSE)